...
const { AdminUserCreateSchema, AdminUserUpdateSchema } = require('../../lib/zodSchemas');
const {
  blank, VALID_ROLES, VALID_USER_TYPES, mapId, normPhone,
  cascadeUserRemoval, auditLog, actorLabel,
} = require('./shared');

/** Map a Mongo duplicate-key error (unique phone index) to the legacy 409. */
const rethrowDupPhone = (message) => (err) => {
  if (err && err.code === 11000) throw new HttpError(409, message);
  throw err;
};
...
module.exports = function adminUsersRouter() {
  ...
  router.get('/', asyncHandler(async (req, res) => {
    ...
    res.json({ ok: true, data: rows.map(mapId), meta: { total, next_cursor } });
  }));
  ...
  router.post('/', validate(AdminUserCreateSchema), asyncHandler(async (req, res) => {
    ...
    if (!VALID_USER_TYPES.includes(user_type)) {
      throw new HttpError(400, `user_type must be one of: ${VALID_USER_TYPES.join(', ')}`);
    }
    if (role === 'super_admin' && !password) {
      throw new HttpError(400, 'password is required for super_admin accounts');
    }
    ...
    await collection('users').insertOne(doc)
      .catch(rethrowDupPhone('Phone number already registered'));
  ...
  router.put('/:id', validate(AdminUserUpdateSchema), asyncHandler(async (req, res) => {
    ...
    if (req.params.id === req.admin.id && blank(role) && role !== 'super_admin') {
      throw new HttpError(400, 'You cannot change your own super_admin role.');
    }
    ...
    if (role === 'super_admin' && !password) {
      ...
      if (!existing || !existing.password_hash) {
        throw new HttpError(400, 'password is required when promoting an account to super_admin');
      }
    }
    ...
    ).catch(rethrowDupPhone('Phone number already in use'));
  ...
  router.delete('/:id', asyncHandler(async (req, res) => {
    if (req.params.id === req.admin.id) {
      throw new HttpError(400, 'Cannot delete your own admin account');
    }
