...
// Focus trap while the radar overlay is open (Tab cycles, Esc closes).
...
<template>
  ...
  <div v-if="isMaximized" class="radar-overlay" @click.self="isMaximized = false">
    <div ref="radarModalEl" class="radar-modal" role="dialog" aria-modal="true" aria-labelledby="radar-modal-title" tabindex="-1">
      <div class="radar-header">
...
.card-caption { font-size:12px; font-weight:700; color:#555; }

/* ── Radar ─────────────────────────────────────────────── */
.radar-half { gap:2px; }
.radar-header { display:flex; align-items:center; justify-content:space-between; }
.radar-max-btn { font-size:12px; font-weight:600; padding:2px 8px; background:#f5f5f5; border:1px solid #d0d0d0; color:#333; cursor:pointer; font-family:inherit; border-radius:2px; }
...
.radar-label { font-size:7px; font-weight:700; fill:#666; text-anchor:middle; dominant-baseline:middle; font-family:inherit; }
...
.radar-legend { display:flex; gap:18px; font-size:12px; font-weight:600; color:#333; justify-content:center; }
...
.dist-legend { display:flex; flex-wrap:wrap; gap:4px 10px; font-size:11px; color:#555; }
